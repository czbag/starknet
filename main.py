...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("16) Mint StarkVerse NFT", mint_starkverse),
            Choice("17) Mint NFT on Pyramid", create_collection_pyramid),
            Choice("18) Transfer", make_transfer),
            Choice("19) Swap tokens to ETH", swap_tokens),
            Choice("20) Use Multiswap", swap_multiswap),
            Choice("21) Use custom routes ", custom_routes),
            Choice("22) Check transaction count", "tx_checker"),
            Choice("23) Exit", "exit"),
        ],
        qmark="⚙️ ",
        pointer="✅ "
